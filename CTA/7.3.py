...
# 减影算法核心分级
# ════════════════════════════════════════════════════════════════════
def fast_subtraction(pre, post_aligned, bone_strength=1.0, vessel_sensitivity=1.0, body=None, cache=None, k_map=None):
    pre_f = pre.astype(np.float32) if pre.dtype != np.float32 else pre
    ...
    if k_map is not None:
        # k_map已由外层修正post，diff_pos的系统性漂移已消除。
        # 用compute_adaptive_threshold替代固定阈值，阈值=纯统计波动项，
        # 颅底厚骨和颅顶薄骨自动获得各自合适的抑制力度。
        adaptive_thresh = compute_adaptive_threshold(
            pre_f, diff_pos, bone_mask, bone_strength, k_map=k_map)
        gain[bone_mask & (diff_pos < adaptive_thresh)] = 0
        gain[bone_mask & (diff_pos >= adaptive_thresh) & (diff_pos < adaptive_thresh * 1.5)] = 0.15 / bone_strength
        gain[bone_mask & (diff_pos >= adaptive_thresh * 1.5)] = 0.3 / bone_strength
    else:
        # 无k_map时保持原固定阈值路径（向后兼容）
...
def quality_subtraction_cached(pre, post_aligned, bone_strength, vessel_sensitivity, cache, k_map=None):
    pre_f = pre.astype(np.float32) if pre.dtype != np.float32 else pre
    post_f = post_aligned.astype(np.float32) if post_aligned.dtype != np.float32 else post_aligned
    diff_pos = np.clip(post_f - pre_f, 0, None)
    ...
    # ── 骨区：有k_map时用自适应阈值，无k_map时保持原固定阈值 ──
    bone_mask_q = pre_f > 150
    if k_map is not None:
        # k修正已消除系统性漂移，阈值纯粹由统计波动决定，
        # ICA保护区在adaptive_thresh内也会正确放行（diff_pos高于阈值）
        adaptive_thresh = compute_adaptive_threshold(
            pre_f, diff_pos, bone_mask_q, bone_strength, k_map=k_map)
        ica = cache.ica_protect if cache.ica_protect is not None else np.zeros_like(pre, dtype=bool)
        gain[bone_mask_q & (diff_pos < adaptive_thresh) & ~ica] = 0
        gain[bone_mask_q & (diff_pos >= adaptive_thresh) & (diff_pos < adaptive_thresh * 1.5) & ~ica] = 0.15 / bone_strength
        gain[bone_mask_q & (diff_pos >= adaptive_thresh * 1.5) & ~ica] = 0.3 / bone_strength
            ...
            for chunk_idx in range(num_chunks):
                ...
                for z in range(z_start, z_end + 1):
                    ...
                    if pre_s.max() > -500:
                        ...
                        # ── 层号与分区参数（k估计需要slices_from_top，提前计算）──
                        ...
                        # ── K修正：逐层估计beam hardening系数，消除骨区系统性漂移 ──
                        # post_bone ≈ k × pre_bone，漂移量 = (k-1) × pre_f。
                        # 非骨区 k_map = 1.0，修正对软组织/血管像素完全无影响。
                        # 修正后的 post_s_k 喂给所有减影引擎，骨残影在源头消除，
                        # 后续的骨抑制增益只需应对真正的配准残差，误杀血管概率大幅降低。
                        _bone_mask_k = pre_s_work > 150
                        k_map, _ = estimate_k_by_zone(
                            pre_s_work, post_s_work, _bone_mask_k, slices_from_top, eff_conv_end)
                        post_s_k = post_s_work - (k_map - 1.0) * np.clip(pre_s_work, 0, None)

                        diff_pos_work = np.clip(post_s_k - pre_s_work, 0, None)
                        ...
                        if opt_level == 'none':
                            res = quality_subtraction_cached(pre_s_work, post_s_k, adj_bone, adj_vessel, cache, k_map=k_map) if opt['quality_mode'] else fast_subtraction(pre_s_work, post_s_k, adj_bone, adj_vessel, body=cache.body, cache=cache, k_map=k_map)
                        elif opt_level == 'light':
                            res = optimized_subtraction_light_v3(pre_s_work, post_s_k, adj_bone, adj_vessel, cache)
                        elif opt_level == 'standard':
                            res = optimized_subtraction_standard_v3(pre_s_work, post_s_k, adj_bone, adj_vessel, cache, spacing_xy, 0.5)
                        else:
                            res = optimized_subtraction_deep_v3(pre_s_work, post_s_k, adj_bone, adj_vessel, cache, spacing_xy, spacing_zyx, 0.5)
