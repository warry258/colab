...
def resource_path(relative_path):
    """获取资源文件的绝对路径，兼容 PyInstaller 单文件打包及中文/系统目录"""
    try:
        base_path = sys._MEIPASS  # PyInstaller 解压的临时目录
    except AttributeError:
        base_path = os.path.dirname(os.path.abspath(__file__))
    return os.path.join(base_path, relative_path)

def _get_short_path(path: str) -> str:
    """将含中文或空格的 Windows 路径转为 8.3 短路径，失败时原样返回。
    
    修复 Win7 32位系统上短路径功能被禁用或部分支持时返回无效路径的问题：
    1. 路径不存在时直接返回原路径，避免 API 转出混合无效路径
    2. API 返回 0（失败）时明确回退，而非依赖 buf.value 是否为空
    3. 扩大缓冲区至 1024，防止深层中文目录结构截断
    4. 转换后二次验证短路径可访问性，无效则回退原路径
    """
    if os.name != 'nt':
        return path
    try:
...
def _sitk_read_image_safe(file_list: list, pixel_type=None) -> 'sitk.Image':
    """
    安全读取 DICOM 文件列表，彻底绕开 SimpleITK 的中文路径问题。

    策略（三级降级）：
      1. 先尝试 8.3 短路径直接调用 sitk.ReadImage（原有逻辑）
      2. 若失败，改用 pydicom 逐帧读取像素 + RescaleSlope/Intercept 修正，
         再通过 sitk.GetImageFromArray 构建 Image，并从第一帧的 DICOM 头
         恢复 Spacing / Origin / Direction，保证后续配准正确。
      3. 若 pydicom 也失败，抛出原始异常供外层捕获。

    参数
    ----
    file_list : list[str]
        已按层序排列的 DICOM 文件绝对路径列表（原始路径，函数内部自行处理短路径）
    pixel_type : sitk pixel type, optional
        默认 sitk.sitkInt16

    返回
    ----
    sitk.Image  （轴序为 X×Y×Z，与 sitk.ReadImage 一致）
    """
    if pixel_type is None:
        pixel_type = sitk.sitkInt16

    # ── 第一级：短路径 + sitk.ReadImage ──
    ...
    # ── 第二级：pydicom 逐帧读取，重建 SimpleITK Image ──
    try:
        ...
        # numpy array shape: (Z, H, W) → sitk expects (Z, Y, X) which is same
        vol = np.stack(slices, axis=0)          # (Z, H, W)
        img = sitk.GetImageFromArray(vol)        # sitk: X=W, Y=H, Z=depth

        # ── 从 DICOM 头恢复空间信息，保证配准精度 ──
        try:
            px_spacing = ds0.PixelSpacing          # [row_spacing, col_spacing] in mm
            row_sp = float(px_spacing[0])
            col_sp = float(px_spacing[1])
            # SliceThickness 或 SpacingBetweenSlices 作为 Z spacing
            z_sp = float(getattr(ds0, 'SpacingBetweenSlices',
                         getattr(ds0, 'SliceThickness', 1.0)))
            img.SetSpacing((col_sp, row_sp, z_sp))   # sitk spacing: (X, Y, Z)
        except Exception:
            pass  # 无法获取就保持默认 1mm

        try:
            iop = ds0.ImageOrientationPatient       # 6个方向余弦
            ipp = ds0.ImagePositionPatient          # 第一层原点
        ...
        # 两级均失败，抛出原始 SimpleITK 异常（含路径信息，便于排查）
        raise RuntimeError(
...
def _get_preset_file_path() -> Path:
    """预设文件路径：打包后从 _MEIPASS 读内置预设，同时在 exe 同目录查找用户自定义预设"""
    if getattr(sys, 'frozen', False):
        # 打包后：用户自定义预设放在 exe 同目录
        exe_dir = Path(sys.executable).parent
        user_preset = exe_dir / "device_presets.json"
        if user_preset.exists():
            return user_preset
        # 无用户文件则返回不存在路径（仅用内置预设）
        return user_preset
    else:
...
# ════════════════════════════════════════════════════════════════════
# 预标注线程：仅读取造影序列，不做配准，供标注窗口浏览选层
# ════════════════════════════════════════════════════════════════════
class PreAnnotationThread(QThread):
    progress = pyqtSignal(int)
    log = pyqtSignal(str)
    # 传出：numpy volume（int16）+ 文件列表（供后续裁切）
        ...
        try:
            ...
            for z, fp in enumerate(f_list):
                ...
                if z == 0:
                    ds0 = None  # 释放第0张缓存
                if z % 50 == 0:
                    self.progress.emit(int(z / total * 100))
            self.progress.emit(100)
...
class ProcessThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.log.emit("=" * 55)
            self.log.emit(f"CTA 减影 V4.5.0  | 优化:{opt_name}")
            ...
            if self.pre_annotation:
                ...
                total_slices = len(pre_files)
                # 复制一份，避免修改主窗口持有的 pre_annotation_result 对象
            ...
            for chunk_idx in range(num_chunks):
                ...
                # 4. 应用预标注 ROI（无需等待，已在启动前确定）
                annotation_data = None
                z_start, z_end = 0, chunk_depth - 1

                if ann_rois:
                    # 将全局 ROI 直接带入本段（局部 ROI 按 z 偏移映射）
                    seg_ann = AnnotationData(
                        z_range_start=0,
                        z_range_end=chunk_depth - 1,
                    ...
                    # 局部 ROI：原始 z 坐标减去本段起始偏移
                    for orig_z, rois in ann_rois.local_rois.items():
                        local_z = orig_z - start_idx
                        if 0 <= local_z < chunk_depth:
                            seg_ann.local_rois[local_z] = rois
                    for orig_z, rois in ann_rois.inverse_local_rois.items():
                        local_z = orig_z - start_idx
                        if 0 <= local_z < chunk_depth:
                            seg_ann.inverse_local_rois[local_z] = rois
                ...
                for z in range(z_start, z_end + 1):
                    ...
                    if pre_s.max() > -500:
                        ...
                        if opt_level == 'none':
                            ...
                            res = optimized_subtraction_standard_v3(pre_s, post_s, adj_bone, adj_vessel, cache, spacing_xy, z_ratio)
                        else:
                            res = optimized_subtraction_deep_v3(pre_s, post_s, adj_bone, adj_vessel, cache, spacing_xy, spacing_zyx, z_ratio)
                ...
                for z in range(z_start, z_end + 1):
                    ...
                    ds.SeriesDescription = f"CTA Sub V4.5.0 [{opt_name}]{ann_tag}"
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.pre_annotation_result: Optional[AnnotationData] = None  # 预标注结果
        self.init_ui()
    def init_ui(self):
        self.setWindowTitle("头颅 CTA 减影-whw 版 V4.5.0")
        ...
        self.log("头颅 CTA 减影-whw 版 V4.5.0 ")
        self.log("=" * 50)
        self.log("V4.5.0 操作说明:")
        self.log("① 扫描目录自动配对  ② 分析特征，拷贝数据（可选）  ③ 预标注（可选，选层/划ROI）  ④ 开始减影")
        self.log("预标注：仅读造影片，标注后仅对选定层段配准减影，大幅降低内存和耗时")
        self.log("  输出图像拥有连续实例号，三维重建无缝合并")
        self.log("=" * 50)
    ...
    def on_pre_ann_ready(self, post_vol: np.ndarray, post_files: list):
        ...
        if result == QDialog.Accepted and dlg.result_accepted:
            ...
            has_roi = bool(self.pre_annotation_result.global_rois or
                           ...
                           any(self.pre_annotation_result.inverse_local_rois.values()))
    ...
    def copy_series_data(self):
        """将当前配对的平扫+增强序列文件拷贝到用户指定目录，并在完成后用资源管理器打开"""
        ...
        class CopyThread(QThread):
            ...
            def __init__(self, jobs):
                super().__init__()
                self.jobs = jobs  # list of (src, dst_dir)
    ...
    def cancel(self):
        if self.proc_thread:
            ...
            # UI 立即锁定，防止重复点击；on_finished 回调后再最终复位
            self.cancel_btn.setEnabled(False)
    ...
    def on_finished(self, success, msg):
        ...
        self.cancel_btn.setEnabled(False)
        self.progress.setValue(0)
        self.proc_thread = None   # 释放线程对象引用
        self.update_output_dir_preview()
        if msg.startswith("⛔"):
            # 取消流程：仅日志提示，不弹窗
            self.log(msg)
...
if __name__ == "__main__":
    ...
    # ── PyInstaller 单文件：将 _MEIPASS 临时目录设为工作目录，
    #    避免 SimpleITK / pydicom 在含中文或空格的路径下展开失败 ──
    if getattr(sys, 'frozen', False):
        # 强制将 CWD 切换到可写的用户临时目录，而非 _MEIPASS（可能在只读系统目录）
        import tempfile
    ...
    if os.name == 'nt':
        try:
            ...
            pass
        # 请求 Windows 使用 UTF-8 代码页（Windows 10 1903+ 支持）
        try:
