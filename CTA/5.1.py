...
class AnnotationData:
    ...
    coil_seed_mask: Optional[np.ndarray] = None  # 3D bool: 魔棒泛洪得到的弹簧圈核心体素
    coil_threshold: float = 1700.0               # 魔棒取核时的窗宽上边界(wc+ww/2)
    z_range_start: int = 0
    z_range_end: int = -1
    series_uid: str = ""
    scale_factor: float = 1.0
    # Willis环手动标定（0-based，对应交互窗口 display_volume 的层索引）
    # willis_layer_top：Willis上界（靠近颅顶，数值较大）
    # willis_layer_bot：Willis下界（靠近颅底，数值较小）
    # None = 未标定，使用程序默认常量 ZONE_CONVEXITY_END / ZONE_WILLIS_END
    ...
    def get_scaled_rois(self, target_scale: float) -> 'AnnotationData':
        ...
        # seed_mask 按新比例重采样（最近邻，保持bool语义）
        if self.coil_seed_mask is not None:
            try:
                scaled.coil_seed_mask = ndimage.zoom(
                    self.coil_seed_mask.astype(np.uint8),
                    (1.0, ratio, ratio), order=0
                ).astype(bool)
            except Exception:
                scaled.coil_seed_mask = self.coil_seed_mask.copy()
        scaled.coil_threshold = self.coil_threshold
        return scaled
...
#
#   颅盖骨区  convexity   : slices_from_top   0 ~ 119   (0  ~ 74.4mm)
#   Willis环区 willis_circle: slices_from_top 120 ~ 167  (75 ~ 104.4mm)
#     ↑ 下界延伸至167层（+7层），纳入鞍底-岩骨尖过渡带
#   颅底区    skull_base  : slices_from_top ≥ 168       (≥ 105mm)
#     （头颈CTA时颈椎段也落于此区，接受强骨抑制，符合预期）
#
#   边界过渡带：各边界两侧 ±8层线性渐变，消除相邻层亮度阶梯。
# ─────────────────────────────────────────────────────────────────
ZONE_CONVEXITY_END   = 120   # 颅盖区结束（含），下一层进入Willis
ZONE_WILLIS_END      = 168   # Willis区结束（含），下一层进入颅底
...
def estimate_k_by_zone(pre_f, post_f, bone_mask, slices_from_top,
    ...
    """
    # 只用HU>200的确定骨质做回归，排除骨边缘部分容积效应像素
    solid_bone = bone_mask & (pre_f > 200)
...
def expand_coil_artifact_3d(pre_vol: np.ndarray, seed_mask_3d: np.ndarray,
    ...
    【返回值更新】返回：(伪影区, 光环搜索区, 核心区) 供后续开展血管救援。
    """
    if log_cb: log_cb("  [弹簧圈] 三维星芒伪影扩展识别与血管救援准备中...")

    pre_f = pre_vol.astype(np.float32)

    # ── 步骤1：搜索区/光环区（核心向外膨胀约10mm xy / 8mm z）──────────────
    ...
    search_zone = ndimage.binary_dilation(seed_mask_3d, structure=struct_ellipsoid)
    search_zone = search_zone & ~seed_mask_3d   # 去掉核心本身

    # ── 步骤2：候选高HU区 ────────────────────────────────────────────
    candidate = search_zone & (pre_f > 150)
    if not candidate.any():
        if log_cb: log_cb("  [弹簧圈] 搜索区内无高HU候选，跳过伪影扩展")
        return np.zeros_like(seed_mask_3d), search_zone, seed_mask_3d

    # ── 步骤3：在全局骨区做开运算，识别厚实骨质 ─────────────────────
    global_bone = pre_f > 150
    struct_open = ndimage.generate_binary_structure(3, 1)  # 3×3×3，6连通
    solid_bone = ndimage.binary_opening(global_bone, structure=struct_open, iterations=2)
    ...
    dark_artifact = search_zone & (pre_f < -20)

    artifact_mask = thin_artifact | dark_artifact

    if log_cb:
        n_art  = int(artifact_mask.sum())
        n_core = int(seed_mask_3d.sum())
        log_cb(f"  [弹簧圈] 核心 {n_core} 体素 | 伪影靶区 {n_art} 体素 | 血管救援光环已就绪")

    return artifact_mask, search_zone, seed_mask_3d
...
class ImageCanvas(QWidget):
    def __init__(self, parent=None):
        ...
        self.wand_preview_mask: Optional[np.ndarray] = None 
    ...
    def paintEvent(self, event):
        ...
        if self.annotations:
            ...
            seed_slice = None
            if self.annotations.coil_seed_mask is not None:
                if self.current_z < self.annotations.coil_seed_mask.shape[0]:
                    seed_slice = self.annotations.coil_seed_mask[self.current_z]
            if seed_slice is None and self.wand_preview_mask is not None:
                seed_slice = self.wand_preview_mask
            if seed_slice is not None and seed_slice.any():
                overlay = np.zeros((seed_slice.shape[0], seed_slice.shape[1], 4), dtype=np.uint8)
                overlay[seed_slice] =[0, 230, 220, 110]
                qimg_ov = QImage(overlay.data, overlay.shape[1], overlay.shape[0],
                                 overlay.shape[1] * 4, QImage.Format_RGBA8888)
                painter.drawImage(0, 0, qimg_ov.copy())
    ...
    def mousePressEvent(self, event):
        ...
        if self.mode == "navigate":
            ...
            if roi and roi.closed:
                ...
                if edge_idx >= 0:
                    ...
                    self.update()
                    if self.on_annotation_changed: self.on_annotation_changed()
        elif self.mode in["global_roi", "local_roi", "global_inverse_roi", "local_inverse_roi", "metal_roi"]:
            if event.button() == Qt.LeftButton and self._is_in_image(ipos):
                if not self.current_polygon:
                    roi_type = "global" if self.mode == "global_roi" else "global_inverse" if self.mode == "global_inverse_roi" else "local_inverse" if self.mode == "local_inverse_roi" else "metal" if self.mode == "metal_roi" else "local"
        ...
        elif self.mode == "coil_wand" and event.button() == Qt.LeftButton:
            if self._is_in_image(ipos) and self.display_volume is not None and self.annotations is not None:
                px = int(ipos.x())
                py = int(ipos.y())
                px = max(0, min(px, self.img_width - 1))
                py = max(0, min(py, self.img_height - 1))
                thr = float(self.wc + self.ww / 2)
                seed_hu = float(self.display_volume[self.current_z, py, px])
                if seed_hu < thr:
                    if self.on_status_message:
                        self.on_status_message(f"种子点HU={seed_hu:.0f} 低于当前阈值{thr:.0f}，请点击更亮区域或调高窗位")
                else:
                    if self.on_status_message:
                        self.on_status_message(f"魔棒泛洪中（阈值≥{thr:.0f} HU）…")
                    QApplication.processEvents()
                    binary = (self.display_volume.astype(np.float32) >= thr)
                    labeled, _ = ndimage.label(binary)
                    seed_label = labeled[self.current_z, py, px]
                    if seed_label > 0:
                        seed_mask_3d = (labeled == seed_label)
                        self.annotations.coil_seed_mask = seed_mask_3d
                        self.wand_preview_mask = seed_mask_3d[self.current_z]
                        n_vox = int(seed_mask_3d.sum())
                        n_slices = int(np.any(seed_mask_3d, axis=(1, 2)).sum())
                        if self.on_status_message:
                            self.on_status_message(
                                f"✓ 弹簧圈核心已捕获：{n_vox} 体素，跨 {n_slices} 层（阈值≥{thr:.0f} HU）")
                    else:
                        if self.on_status_message:
                            self.on_status_message("种子点落在阈值以下，未找到连通体")
                    self.update()

            elif event.button() == Qt.RightButton:
                if self.annotations is not None:
                    self.annotations.coil_seed_mask = None
                self.wand_preview_mask = None
                if self.on_status_message:
                    self.on_status_message("魔棒结果已清除")
                self.update()
    ...
    def _save_current_polygon_cross_slice(self):
        ...
        if roi_type == "global":
            ...
            for z in range(min_z, max_z + 1):
                ...
                self.annotations.metal_rois[z].append(roi_copy)
        elif roi_type == "local":
            for z in range(min_z, max_z + 1):
                roi_copy = self.current_polygon.copy()
                roi_copy.slice_index = z
    ...
    def _try_merge_rois(self):
        ...
        if self.current_z in self.annotations.metal_rois:
            rois = self.annotations.metal_rois[self.current_z]
            if len(rois) > 1:
                old_count = len(rois)
                self.annotations.metal_rois[self.current_z] = merge_overlapping_polygons(rois, "metal", self.current_z)
                merged_count += old_count - len(self.annotations.metal_rois[self.current_z])
    ...
    def delete_last_roi(self):
        if not self.annotations: return
        if self.current_z in self.annotations.metal_rois and self.annotations.metal_rois[self.current_z]:
...
class InteractiveAnnotationDialog(QDialog):
    ...
    def init_ui(self):
        ...
        tl.addWidget(self._sep())
        # [瘤夹备用] tl.addWidget(self._section_label("弹簧圈（魔棒取核）"))
        # [瘤夹备用] btn_wand = QPushButton("魔棒取核 - 点击")
        # [瘤夹备用] btn_wand.setCheckable(True)
        # [瘤夹备用] btn_wand.setStyleSheet("""
            # [瘤夹备用] QPushButton { background: #001a33; color: #00eeff; border: 1px solid #00aacc; border-radius: 4px; padding: 8px; }
            # [瘤夹备用] QPushButton:hover { background: #002244; }
            # [瘤夹备用] QPushButton:checked { background: #005577; border-color: #00eeff; color: white; }
        # [瘤夹备用] """)
        # [瘤夹备用] btn_wand.clicked.connect(lambda _, m="coil_wand": self.set_mode(m))
        # [瘤夹备用] self.mode_group.addButton(btn_wand)
        # [瘤夹备用] tl.addWidget(btn_wand)
        # [瘤夹备用] self.mode_buttons["coil_wand"] = btn_wand
        ...
        # ── Willis 环手动标定行（与"处理范围"行完全一致的交互逻辑）────
        # 滑动到目标层面后点击"设为上界/下界"自动回填；
        # 也可直接点击 SpinBox 箭头手动调整。
        # 不填写（保持默认值）则以程序默认常量为准。
        r3 = QHBoxLayout()

        lbl_w = QLabel("Willis 环：")
        lbl_w.setStyleSheet("color: white; font-size: 9pt;")
        r3.addWidget(lbl_w)

        n_total = self.display_volume.shape[0]
        default_top = max(1, int(n_total * 0.60))
        default_bot = max(1, int(n_total * 0.45))
        ...
        self.willis_top.setValue(default_top)
        r3.addWidget(self.willis_top)

        self._willis_btn_top = QPushButton("设为上界")
        self._willis_btn_top.setStyleSheet(_btn)
        self._willis_btn_top.clicked.connect(
            lambda: self.willis_top.setValue(self.canvas.current_z + 1))
        ...
        self.willis_bot.setValue(default_bot)
        r3.addWidget(self.willis_bot)

        self._willis_btn_bot = QPushButton("设为下界")
        self._willis_btn_bot.setStyleSheet(_btn)
        self._willis_btn_bot.clicked.connect(
            lambda: self.willis_bot.setValue(self.canvas.current_z + 1))
        r3.addWidget(self._willis_btn_bot)

        # willis_check 仍保留（accept_annotation 读取它），但不控制 enable/disable
        self.willis_check = QCheckBox("")
        self.willis_check.setVisible(False)   # 隐藏，仅作数据标志
        ...
        # [瘤夹备用] help_label = QLabel("1-4 切换模式\n5-7 顶点编辑\n8-9 反向保留\n0 金属保护\n= 魔棒取核\nEsc 取消\nDel 删除\nCtrl+Z 撤销")
    ...
    def clear_slice(self):
        ...
        if has_local or has_inv or has_metal:
            mb = QMessageBox(self)
    ...
    def accept_annotation(self):
        ...
        self.annotations.coil_threshold = float(self.canvas.wc + self.canvas.ww / 2)
        # Willis环标定：上界层号必须大于下界层号才有效（0-based存储）
    ...
    def keyPressEvent(self, event):
        mode_keys = {Qt.Key_1: "navigate", Qt.Key_2: "window", Qt.Key_3: "global_roi", Qt.Key_4: "local_roi",
                     ...
                     # [瘤夹备用] Qt.Key_Equal: "coil_wand"  ← 恢复时把上行末尾}移到此处行尾
...
class ProcessThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.log.emit("=" * 55)
            self.log.emit(f"CTA 减影 V4.5.5 (三维形态学终极破局版) | 优化:{opt_name}")
            ...
            original_total_slices = total_slices
            ...
            for chunk_idx in range(num_chunks):
                ...
                if ann_rois:
                    ...
                    if ann_rois.coil_seed_mask is not None:
                        # coil_seed_mask的Z轴与post_vol对齐（Z=0=原序列第0层）
                        # 预标注裁切了az0~az1，chunk在裁切后的start_idx处，
                        # 所以在原序列坐标系中对应 az0+start_idx
                        full = ann_rois.coil_seed_mask
                        seg_z0c = max(0, az0 + start_idx)
                        seg_z1c = min(full.shape[0], az0 + start_idx + chunk_depth)
                        if seg_z0c < seg_z1c:
                            sliced = full[seg_z0c:seg_z1c]
                            # 若切出的层数不足chunk_depth（边界情况），pad到一致
                            if sliced.shape[0] < chunk_depth:
                                pad = np.zeros((chunk_depth - sliced.shape[0],
                                                sliced.shape[1], sliced.shape[2]), dtype=bool)
                                sliced = np.concatenate([sliced, pad], axis=0)
                            seg_ann.coil_seed_mask = sliced
                        seg_ann.coil_threshold = ann_rois.coil_threshold
                        self.log.emit(f"[弹簧圈] seed_mask切片：原序列Z={seg_z0c}~{seg_z1c}，"
                                      f"有效体素={int(seg_ann.coil_seed_mask.sum()) if seg_ann.coil_seed_mask is not None else 0}")
                ...
                # ── Willis环边界：医生标定优先，否则使用程序默认常量 ──────────
                # annotation_data.willis_layer_top/bot 是 display_volume 的0-based层索引，
                # 需经 az0 偏移 → original_global_z → slices_from_top 完成坐标转换。
                eff_conv_end  = ZONE_CONVEXITY_END   # 默认：120层
                eff_willis_end = ZONE_WILLIS_END      # 默认：168层
                if (annotation_data is not None and
                    ...
                    # willis_layer_top/bot 是医生在完整序列上标定的 0-based 层号
                    # （display_volume Z轴未缩放，直接等于完整序列坐标），
                    # 不需要再加 az0，否则会双重偏移。
                    og_top = annotation_data.willis_layer_top
                    og_bot = annotation_data.willis_layer_bot
                    # 层号大（靠顶）→ slices_from_top 小 = 颅盖/Willis边界
                    # 层号小（靠底）→ slices_from_top 大 = Willis/颅底边界
                    eff_conv_end  = max(0, (original_total_slices - 1) - og_top)
                    eff_willis_end = max(eff_conv_end + 1,
                                        (original_total_slices - 1) - og_bot)
                    self.log.emit(
                        f"[Willis] 医生标定：上界第{annotation_data.willis_layer_top+1}层"
                        f"→ sft={eff_conv_end}，"
                        f"下界第{annotation_data.willis_layer_bot+1}层"
                        f"→ sft={eff_willis_end}"
                        f"（程序默认 {ZONE_CONVEXITY_END}/{ZONE_WILLIS_END}）")
                else:
                    self.log.emit(
                        f"[Willis] 使用程序默认边界"
                ...
                # ── 弹簧圈星芒伪影：循环前一次性3D扩展 ─────────────────────
                coil_artifact_vol = None
                coil_halo_vol     = None
                coil_core_vol     = None
                has_coil_seed = (annotation_data is not None and
                                 annotation_data.coil_seed_mask is not None and
                                 annotation_data.coil_seed_mask.any())
                if has_coil_seed:
                    seed_3d = annotation_data.coil_seed_mask
                    if seed_3d.shape != pre_vol.shape:
                        # Z轴维度不同时：优先用切片对齐（分段处理已在seg_ann构建时截取），
                        # XY维度不一致时才对Z轴以外做zoom，避免Z轴压缩丢失弹簧圈体素。
                        if seed_3d.shape[0] != pre_vol.shape[0]:
                            nz = pre_vol.shape[0]
                            if seed_3d.shape[0] >= nz:
                                seed_3d = seed_3d[:nz]
                            else:
                                pad = np.zeros((nz - seed_3d.shape[0],
                                                seed_3d.shape[1], seed_3d.shape[2]), dtype=bool)
                                seed_3d = np.concatenate([seed_3d, pad], axis=0)
                        if seed_3d.shape[1] != pre_vol.shape[1] or seed_3d.shape[2] != pre_vol.shape[2]:
                            zoom_f = (1.0,
                                      pre_vol.shape[1] / seed_3d.shape[1],
                                      pre_vol.shape[2] / seed_3d.shape[2])
                            seed_3d = ndimage.zoom(seed_3d.astype(np.uint8), zoom_f, order=0).astype(bool)
                    n_seed_vox = int(seed_3d.sum())
                    self.log.emit(f"[弹簧圈] seed_mask对齐后：{seed_3d.shape}，有效体素 {n_seed_vox}")
                    coil_artifact_vol, coil_halo_vol, coil_core_vol = expand_coil_artifact_3d(
                        pre_vol, seed_3d, spacing_zyx, log_cb=self.log.emit)
                    self.log.emit(f"[弹簧圈] 三维伪影扩展与光环生成完成，进入逐层减影")
                ...
                for z in range(z_start, z_end + 1):
                    ...
                    if pre_s.max() > -500:
                        ...
                        if has_metal and z in annotation_data.metal_rois:
                            ...
                            if masks: metal_mask_z = np.any(np.stack(masks), axis=0)

                        if metal_mask_z is not None:
                            raw_metal_core = metal_mask_z & (pre_s > 2800)  # 铂合金真实HU>3000，不取梯度带
                            expanded_metal_core = ndimage.binary_dilation(raw_metal_core, iterations=2) & metal_mask_z  # 3000HU本体膨胀2px吃掉周边皮
                            strict_potential_bone = pre_s > 400
                            strict_solid_bone = ndimage.binary_opening(strict_potential_bone, iterations=2)
                            strict_solid_bone = ndimage.binary_dilation(strict_solid_bone, iterations=2) & strict_potential_bone
                            bright_artifact = metal_mask_z & (pre_s > 120) & (~strict_solid_bone) & (~expanded_metal_core)
                            dark_artifact_m = metal_mask_z & (pre_s < -20)
                            
                            metal_core_mask = expanded_metal_core
                            raw_metal_artifact = bright_artifact | dark_artifact_m
                            
                            # 【核心修复：手工 ROI 的光环血管救援】
                            # 无论是在死硬骨区还是普通金属伪影区，只要具备强烈的造影剂特征 (real_diff > 45)，一律救援为软组织！
                            rescue_mask_m = metal_mask_z & (real_diff > rescue_threshold) & (~metal_core_mask)
                            pre_s_work[rescue_mask_m] = 40.0
                            post_s_work[rescue_mask_m] = 40.0 + real_diff[rescue_mask_m]
                            
                            # 剔除掉被救援的真血管后，剩下的才是必杀的纯伪影
                            metal_artifact_mask = raw_metal_artifact & ~rescue_mask_m

                        # ── 弹簧圈：光环救援与精确点杀 ──
                        coil_final_mask = None
                        coil_artifact_mask_z = None
                        if has_coil_seed and coil_artifact_vol is not None:
                            art_z  = coil_artifact_vol[z] if z < coil_artifact_vol.shape[0] else None
                            halo_z = coil_halo_vol[z]     if z < coil_halo_vol.shape[0]     else None
                            core_z = coil_core_vol[z]     if z < coil_core_vol.shape[0]     else None
                            
                            if halo_z is not None and halo_z.any():
                                # 【核心修复：弹簧圈光环血管救援】
                                # 哪怕该像素被形态学划分为伪影(art_z)，只要它展现出真血管强化，一律无差别救援！
                                rescue_mask_c = halo_z & (real_diff > rescue_threshold)
                                if core_z is not None:
                                    rescue_mask_c &= ~core_z
                                    
                                pre_s_work[rescue_mask_c] = 40.0
                                post_s_work[rescue_mask_c] = 40.0 + real_diff[rescue_mask_c]
                                
                                if art_z is not None:
                                    # 剔除掉被救援的真血管后，剩下的作为最终必杀伪影掩膜
                                    coil_artifact_mask_z = art_z & ~rescue_mask_c
                                    
                            if core_z is not None and core_z.any():
                                coil_final_mask = core_z
                                pre_s_work[core_z] = 40.0
                                post_s_work[core_z] = 40.0

                        # 把经过救援的干净 post_s_work 和 pre_s_work 喂给减影引擎
                        ...
                        # ── 解剖分区（绝对层数，与扫描范围/裁切/分段无关）──────
                        # original_global_z：当前层在原始完整序列中的索引（0=最底层）
                        # original_total_slices：裁切前的完整序列层数（全程固定不变）
                        # slices_from_top：距颅顶的绝对层数（0=顶层vertex，向下递增）
                        ...
                        # z_ratio 仅保留供 SliceCache 内部兜底使用，不再参与分区判断
                        z_ratio = original_global_z / max(1, original_total_slices - 1)
                        cache.z_ratio = z_ratio
                        cache.slices_from_top = slices_from_top
                        cache.eff_conv_end = eff_conv_end
                        bone_zones = detect_problematic_bone_zones_v2(
                            pre_s_work, slices_from_top, eff_conv_end)
                        ...
                        if opt_level == 'none':
                            ...
                            res = optimized_subtraction_standard_v3(pre_s_work, post_s_work, adj_bone, adj_vessel, cache, spacing_xy, z_ratio)
                        else:
                            res = optimized_subtraction_deep_v3(pre_s_work, post_s_work, adj_bone, adj_vessel, cache, spacing_xy, spacing_zyx, z_ratio)

                        if zone == 'skull_base':
                            # 颅底区：全套骨抑制（petrous/skull_base骨区/眶板均激活）
                            ...
                            # Willis环区：adj_bone已×0.75降强，仅保留petrous/sharp_edge/
                            # orbital_plate的轻度抑制，skull_base骨区在此层面不激活
                            # （detect_problematic_bone_zones_v2已按绝对层数控制激活）
                            gain = np.ones_like(res, dtype=np.float32)
                            gain = enhanced_bone_suppression_v299(pre_s_work, diff_pos_work, gain, bone_zones, adj_bone, adj_vessel)
                            res = res * gain
                        else:
                            # 颅盖区：仅convexity_thin/sharp_edge激活，颅底骨区不存在
                            gain = np.ones_like(res, dtype=np.float32)
                            gain = enhanced_bone_suppression_v299(pre_s_work, diff_pos_work, gain, bone_zones, adj_bone, adj_vessel)
                            res = res * gain
                        ...
                        # ── [V4.5.5] 微弱血管非线性提亮 ─────────────────────────────
                        # 针对 AChA、脉络膜后动脉、深穿支等减影后 CT 值偏低（50~150 HU）
                        # 的细小血管，用余弦平滑曲线将信号提升至 VR 显示门槛以上。
                        # 设计原则：
                        #   1. 两端（50 / 150 HU）增益=1.0，区间中心（~100 HU）增益最大，
                        #      彻底消除硬边界在 VR/MIP 上产生的亮环伪影。
                        #   2. 提亮前做 2D 连通性过滤（≥3 像素），孤立噪点不提亮。
                        #   3. 仅作用于体内像素（cache.body），体外不受影响。
                        _WV_LO   = 50.0    # 提亮区间下限 HU
                        _WV_HI   = 150.0   # 提亮区间上限 HU
                        _WV_GAIN = 1.35    # 区间中心最大增益
                        ...
                        if _wv_mask.any():
                            ...
                            if _n > 0:
                                ...
                                if len(_valid_labels) > 0:
                                    ...
                                    _t = ((res_scaled[_valid_mask] - _WV_LO)
                                          / (_WV_HI - _WV_LO))
                                    _smooth_gain = (1.0 + (_WV_GAIN - 1.0)
                                                    * 0.5 * (1.0 - np.cos(_t * np.pi)))
                                    res_scaled[_valid_mask] = (res_scaled[_valid_mask]
                                                               * _smooth_gain)

                        # 【最后防线：对没有血管救援护体的纯伪影，实施精准点杀清零！】
                        if coil_artifact_mask_z is not None and coil_artifact_mask_z.any():
                            res_scaled[coil_artifact_mask_z] = 0.0
                            
                        if metal_artifact_mask is not None and metal_artifact_mask.any():
                            res_scaled[metal_artifact_mask] = 0.0

                        # 【三维立体打击 3/3 后置：金属核心幽灵化，随机噪声模拟弹簧圈粗糙质感】
                        if metal_core_mask is not None:
                            n_metal = int(metal_core_mask.sum())
                            res_scaled[metal_core_mask] = np.random.normal(
                                200, 60, size=n_metal).clip(50, 400).astype(np.float32)

                        # 【弹簧圈幽灵化：全核心50~300HU随机噪声】
                        # 粗糙颗粒感是弹簧圈的固有特征，MIP/VR上可与光滑血管明确区分，
                        # 不会被误认为完整动脉瘤。50~300HU宽范围保证MIP可见且VR半透明。
                        if coil_final_mask is not None and coil_final_mask.any():
                            n_core = int(coil_final_mask.sum())
                            res_scaled[coil_final_mask] = np.random.normal(
                                200, 60, size=n_core).clip(50, 400).astype(np.float32)
                ...
                for z in range(z_start, z_end + 1):
                    ...
                    # 【弹簧圈核心最终覆写：逐层写盘时注入随机噪声，在所有后处理之后，不受cleanup影响】
                    if has_coil_seed and annotation_data is not None and annotation_data.coil_seed_mask is not None:
                        _seed = annotation_data.coil_seed_mask
                        if z < _seed.shape[0]:
                            _seed_z = _seed[z]
                            if _seed_z.shape != pix.shape[0:2]:
                                _seed_z = ndimage.zoom(_seed_z.astype(np.uint8),
                                                       (pix.shape[0] / _seed_z.shape[0],
                                                        pix.shape[1] / _seed_z.shape[1]),
                                                       order=0).astype(bool)
                            n_px = int(_seed_z.sum())
                            if n_px > 0:
                                pix[_seed_z] = np.random.uniform(50, 300, size=n_px).astype(np.float32)
                    ...
                    ds.SeriesDescription = f"CTA Sub V4.5.5[{opt_name}]{ann_tag}"
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.init_ui()
    def init_ui(self):
        self.setWindowTitle("头颅 CTA 减影-whw 版 V4.5.5")
        ...
        ann_lay = QHBoxLayout()
        self.pre_ann_btn = QPushButton("预标注（选层/划ROI）")
        self.pre_ann_btn.setStyleSheet("background:#8e44ad;color:white;border-radius:4px;padding:8px 16px;font-weight:bold;min-height:34px;")
        self.pre_ann_btn.setEnabled(False)
        self.pre_ann_btn.clicked.connect(self.start_pre_annotation)
        self.pre_ann_status = QLabel("未标注（将处理全部层）")
        self.pre_ann_status.setStyleSheet("color:#888;font-size:9pt;")
        ann_lay.addWidget(self.pre_ann_btn)
        ann_lay.addWidget(self.pre_ann_status)
        ann_lay.addStretch()
        ...
        self.log("头颅 CTA 减影-whw 版 V4.5.5 操作步骤")
        self.log("=" * 50)             
        self.log("1. 扫描目录自动配对")
    ...
    def scan_directory(self):
        ...
        self.pre_ann_status.setText("未标注（将处理全部层）")
        self.pre_ann_status.setStyleSheet("color:#888;font-size:9pt;")
    ...
    def start_processing(self):
        ...
        # finished 是 QThread 内置信号，run() 返回后由 Qt 自动发出。
        # 连接 deleteLater 确保 C++ 对象在线程彻底退出后才被销毁，
    ...
    def on_pre_ann_ready(self, post_vol: np.ndarray, post_files: list):
        ...
        if result == QDialog.Accepted and dlg.result_accepted:
            ...
            self.pre_annotation_result = None
            self.pre_ann_status.setText("已取消（将处理全部层）")
            self.pre_ann_status.setStyleSheet("color:#888;font-size:9pt;")
            self.log("预标注已取消")
            self.start_btn.setEnabled(True)
