...
def trim_to_common_z_range(files_a, files_b, log_cb=None):
    """
    当两序列层数不等时（如头颈部平扫609层 vs 头颅增强249层），
    取较长序列的【末尾 N 层】与较短序列对齐后返回。

    临床依据：技师头颈部平扫通常从颈部（足侧）扫向头顶，
    头颅部分位于序列末尾；而补扫的头颅增强恰好对应这段范围。
    直接取尾部，层数精确相等，无需任何坐标运算，也不丢层。
    """
    if len(files_a) == len(files_b):
        return files_a, files_b

    n_a, n_b = len(files_a), len(files_b)
    n = min(n_a, n_b)          # 以较短序列的层数为准
    ...
    if n_a > n_b:
        ...
        trimmed_b = files_b
        if log_cb:
            log_cb(f"  [层数对齐] 平扫({n_a}层) > 增强({n_b}层)")
            log_cb(f"  [层数对齐] 取平扫末尾 {n} 层（丢弃前 {n_a - n} 层颈部数据）→ 两序列均 {n} 层")
    else:
        # B 更长（增强头颈部，少见但兼容），取其末尾 n 层
        trimmed_a = files_a
        trimmed_b = files_b[-n:]
        if log_cb:
            log_cb(f"  [层数对齐] 增强({n_b}层) > 平扫({n_a}层)")
            log_cb(f"  [层数对齐] 取增强末尾 {n} 层（丢弃前 {n_b - n} 层颈部数据）→ 两序列均 {n} 层")
...
def build_z_matched_file_pairs(pre_files_tuples, post_files_tuples, log_cb=None):
    """
    将 SeriesInfo.files (instance_number, path) 形式的两组文件，
    按Z坐标对齐，返回可用于 ParamAnalyzeThread 的公共 instance 列表和字典。
    优先用instance number交集；若交集为空则改用Z坐标最近邻匹配。
    返回: (common_keys, pre_dict, post_dict)
    """
    ...
    # instance number无交集，改用Z坐标匹配
    if log_cb:
        log_cb("  [参数分析] instance number无重叠，改用Z坐标匹配采样层")
...
def apply_roi_masks_to_volume(volume: np.ndarray, annotations: AnnotationData, 
    ...
    """
    将标注的 ROI 应用到体数据：
    - global_rois / local_rois        : ROI 内部置 0（清除挡板/骨质）
    - inverse_global_rois / inverse_local_rois : ROI 外部置 0（仅保留框内区域）
    执行顺序：先做反向裁剪（保留），再做正向清除（挖除）
    """
    if log_cb:
        log_cb("  [ROI应用] 生成掩膜中...")
    ...
    # ── 预计算全局正向掩膜 ──
    global_masks = [generate_roi_mask_from_polygon(roi, (h, w))
                    for roi in annotations.global_rois]
    combined_global = np.any(np.stack(global_masks), axis=0) if global_masks else None
    if log_cb and global_masks:
        log_cb(f"  [全局ROI-清除] {len(global_masks)} 个")

    # ── 预计算全局反向掩膜（保留区域的并集）──
    inv_global_masks = [generate_roi_mask_from_polygon(roi, (h, w))
                        for roi in annotations.inverse_global_rois]
    combined_inv_global = np.any(np.stack(inv_global_masks), axis=0) if inv_global_masks else None
    if log_cb and inv_global_masks:
        log_cb(f"  [全局ROI-反向] {len(inv_global_masks)} 个（框外丢弃）")

    processed_local_count = 0
    processed_inv_local_count = 0

    for z in range(z_start, min(z_end + 1, depth)):

        # 1. 反向全局：框外全部置0（保留框内）
        if combined_inv_global is not None:
            result[z][~combined_inv_global] = 0

        # 2. 反向局部：该层有单独的保留框
        if z in annotations.inverse_local_rois and annotations.inverse_local_rois[z]:
            inv_local_masks = [generate_roi_mask_from_polygon(roi, (h, w))
                               for roi in annotations.inverse_local_rois[z]]
            if inv_local_masks:
                combined_inv_local = np.any(np.stack(inv_local_masks), axis=0)
                result[z][~combined_inv_local] = 0
                processed_inv_local_count += 1
        ...
        if z in annotations.local_rois and annotations.local_rois[z]:
            local_masks = [generate_roi_mask_from_polygon(roi, (h, w))
    ...
    if log_cb:
        log_cb(f"  [局部ROI-清除] 处理 {processed_local_count} 层")
        if processed_inv_local_count:
            log_cb(f"  [局部ROI-反向] 处理 {processed_inv_local_count} 层")
...
class ImageCanvas(QWidget):
        ...
        if self.annotations:
            if self.mode not in ("local_roi", "local_inverse_roi"):
                ...
                for roi in self.annotations.inverse_global_rois:
                    self._draw_polygon(painter, roi, QColor(0, 220, 220, 220),
                                       roi == self.selected_roi, roi == self.hover_roi,
                                       inverse=True)
            if self.current_z in self.annotations.local_rois:
            ...
            if self.current_z in self.annotations.inverse_local_rois:
                for roi in self.annotations.inverse_local_rois[self.current_z]:
                    self._draw_polygon(painter, roi, QColor(0, 200, 255, 220),
                                       roi == self.selected_roi, roi == self.hover_roi,
                                       inverse=True)
    ...
    def _draw_polygon(self, painter: QPainter, roi: PolygonROI, color: QColor,
                      selected=False, hover=False, drawing=False, inverse=False):
        if not roi.points:
            return
        ...
        if self.mode == "navigate":
            ...
            if event.button() == Qt.LeftButton and self._is_in_image(ipos):
                if not self.current_polygon:
                    if self.mode in ("global_roi", "global_inverse_roi"):
                        roi_type = "global_inverse" if self.mode == "global_inverse_roi" else "global"
                    else:
                        roi_type = "local_inverse" if self.mode == "local_inverse_roi" else "local"
    ...
    def _save_current_polygon_cross_slice(self):
        ...
        min_z, max_z = state.get_slice_range()
        slice_count = state.get_slice_count()
        roi_type = self.current_polygon.roi_type  # global / local / global_inverse / local_inverse

        if roi_type == "global":
            self.annotations.global_rois.append(self.current_polygon)
            if self.on_status_message:
                self.on_status_message("已添加全局ROI（清除）")

        elif roi_type == "global_inverse":
            self.annotations.inverse_global_rois.append(self.current_polygon)
            if self.on_status_message:
                self.on_status_message("已添加全局反向ROI（框外丢弃）")
            ...
            if self.on_status_message:
                if slice_count > 1:
                    self.on_status_message(f"已将清除ROI应用到第 {min_z + 1} ~ {max_z + 1} 层 (共 {slice_count} 层)")
                else:
                    self.on_status_message(f"已添加第 {min_z + 1} 层的清除ROI")
            ...
            if self.on_status_message:
                if slice_count > 1:
                    self.on_status_message(f"已将保留ROI应用到第 {min_z + 1} ~ {max_z + 1} 层 (共 {slice_count} 层)")
                else:
                    self.on_status_message(f"已添加第 {min_z + 1} 层的保留ROI")
    ...
    def _try_merge_rois(self):
        ...
        if len(self.annotations.inverse_global_rois) > 1:
            ...
            self.annotations.inverse_global_rois = merge_overlapping_polygons(
                self.annotations.inverse_global_rois, "global_inverse", 0)
        ...
        if self.current_z in self.annotations.inverse_local_rois:
            ...
            if len(rois) > 1:
                old_count = len(rois)
                self.annotations.inverse_local_rois[self.current_z] = merge_overlapping_polygons(
                    rois, "local_inverse", self.current_z)
    ...
    def delete_last_roi(self):
        if not self.annotations:
            return
        # 优先删除当前层的局部反向ROI，再删局部正向，再删全局反向，最后全局正向
...
class InteractiveAnnotationDialog(QDialog):
    ...
    def init_ui(self):
        ...
        inverse_modes = [
            ("保留区域-全局", "global_inverse_roi"),
            ("保留区域-单层", "local_inverse_roi"),
        ]
        for text, mode in inverse_modes:
            btn = QPushButton(text)
            btn.setCheckable(True)
            btn.setStyleSheet("""
                QPushButton {
    ...
    def clear_slice(self):
        ...
        has_normal = z in self.annotations.local_rois and self.annotations.local_rois[z]
        has_inverse = z in self.annotations.inverse_local_rois and self.annotations.inverse_local_rois[z]
        if has_normal or has_inverse:
            if QMessageBox.question(self, "确认", f"清除第 {z + 1} 层的所有ROI?") == QMessageBox.Yes:
                if has_normal:
                    self.annotations.local_rois[z].clear()
                if has_inverse:
                    self.annotations.inverse_local_rois[z].clear()
    ...
    def keyPressEvent(self, event):
        mode_keys = {
            ...
            Qt.Key_8: "global_inverse_roi",   # ★ 反向全局
            Qt.Key_9: "local_inverse_roi",    # ★ 反向单层
        }
...
class ProcessThread(QThread):
    ...
    def run(self):
        try:
            ...
            # ★ 层数不等时（如头颈部平扫601层 vs 头颅增强249层），
            #   按Z轴物理坐标裁剪多余层面，保留重叠部分后再读取 ★
            if len(pre_files) != len(post_files):
                ...
                if not pre_files or not post_files:
                    self.finished_signal.emit(False, "层数对齐失败：两序列无有效重叠范围，请手动选择范围相同的序列")
                    return
                self.log.emit(f"  ✓ 对齐完成 → 两序列均使用 {len(pre_files)} 层")
            ...
            if self.enable_annotation:
                ...
                if self.annotation_result:
                    ...
                    self.log.emit(f"  [全局ROI-清除] {len(annotation_data.global_rois)} 个")
                    ...
                    self.log.emit(f"  [局部ROI-清除] {local_count} 个 (分布在 {local_slices} 层)")
                    inv_g = len(annotation_data.inverse_global_rois)
                    inv_lc = sum(len(rois) for rois in annotation_data.inverse_local_rois.values())
                    inv_ls = len([z for z, r in annotation_data.inverse_local_rois.items() if r])
                    if inv_g or inv_lc:
                        self.log.emit(f"  [全局ROI-保留] {inv_g} 个（框外丢弃）")
                        self.log.emit(f"  [局部ROI-保留] {inv_lc} 个 (分布在 {inv_ls} 层)")
            ...
            if annotation_data and (annotation_data.global_rois or any(annotation_data.local_rois.values())
                                    or annotation_data.inverse_global_rois or any(annotation_data.inverse_local_rois.values())):
