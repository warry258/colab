...
[核心重构] 三维立体打击逻辑：
  ...
  2. 提取细长放射状金属伪影，强行降维至 40 HU（血管不再断裂）
...
def apply_roi_masks_to_volume(volume: np.ndarray, annotations: AnnotationData, z_start: int, z_end: int, log_cb=None) -> np.ndarray:
    ...
    for z in range(z_start, min(z_end + 1, depth)):
        # ── 反向保留（保留框内）：先把全局框与局部框合并成并集，再一刀置零框外 ──
        # 修复：原逻辑对全局框和局部框各执行一次 result[~mask]=0，
        # 等同于对图像连续做两次过筛，最终保留的是二者的交集。
        # 若两框不重叠，该层全黑。医学语义应为并集，此处统一修正。
        inv_parts = []
        ...
        if inv_parts:
            combined_inv = np.any(np.stack(inv_parts), axis=0)  # 并集，只做一次置零
...
class ImageCanvas(QWidget):
    ...
    def set_mode(self, mode: str):
        ...
        self._clear_edit_state()
        self.update()
        cursors = {"navigate": Qt.OpenHandCursor, "window": Qt.SizeAllCursor, "move_vertex": Qt.PointingHandCursor, "delete_vertex": Qt.ForbiddenCursor}
...
class ProcessThread(QThread):
    ...
    def __init__(self, pre_series, post_series, output_dir, options, pre_annotation=None):
        ...
        # pre_annotation: AnnotationData 或 None（含 z_range_start/z_range_end/rois）
        ...
        try:
            ...
            def get_files(series):
                ...
                if f_list and len(f_list) == series.file_count:
                    return list(f_list)
                # 回退：按IPP Z坐标物理排序，避免InstanceNumber方向与Z轴方向不一致
                # 某些CT机器（GE/Philips脚→头扫描）InstanceNumber与IPP方向相反，
                # 按InstanceNumber排序传给ITK后报Non uniform sampling WARNING并可能崩溃。
            ...
            # 保存截断前的原始总层数，供解剖定位（z_ratio）使用。
            # 预标注裁切后 total_slices 会被重置为裁切段长度，
            # 若用裁切后的值计算 z_ratio，头顶段起点会被误判为颅底。
            ...
            for chunk_idx in range(num_chunks):
                ...
                if ann_rois:
                    ...
                    # 局部 ROI：原始 z 坐标减去整体裁切偏移(az0)再减去本段偏移(start_idx)
                ...
                if has_metal:
                    self.log.emit(f"[金属保护] 已启用，启动三维立体打击逻辑！(覆盖 {len(annotation_data.metal_rois)} 层)")
                ...
                for z in range(z_start, z_end + 1):
                    ...
                    if pre_s.max() > -500:
                        ...
                        if metal_mask_z is not None:
                            ...
                            # 【三维立体打击 3/3 前置：寻核与吞噬】
                            # 提取 >1700 的极高密度核心
                            raw_metal_core = metal_mask_z & (pre_s > 1700)
                            # 膨胀2个像素，一口气“吃掉”800~1600 HU 的过渡梯度晕影(外壳)
                            expanded_metal_core = ndimage.binary_dilation(raw_metal_core, iterations=2) & metal_mask_z
                            
                            # 【三维立体打击 1/3：严苛验骨与强力断桥，保护基底动脉】
                            # 在保护区内废弃150的低阈值，提高门槛至 >400，防止伪影与斜坡骨质连桥
                            strict_potential_bone = pre_s > 400
                            
                            # 开运算提取厚实颅底骨（真骨头）。加大力度打断伪影与颅底骨的连桥
                            ...
                            # 重新定位伪影：>120 且不是膨胀后的核心，也不是严苛验骨后存活的真骨头
                            bright_artifact = metal_mask_z & (pre_s > 120) & (~strict_solid_bone) & (~expanded_metal_core)
                            dark_artifact = metal_mask_z & (pre_s < -20)
                            
                            # 【三维立体打击 2/3：血管保卫战（执行死刑）】
                            # 将伪影外壳、暗带以及膨胀后的金属核心，全部强行压平为 40 HU 软组织！
                            # 这样基底动脉的减影值就会暴增，底层引擎将其作为血管完美保留，不再断裂。
                            pre_s_work[bright_artifact] = 40
                            pre_s_work[dark_artifact] = 40
                            pre_s_work[expanded_metal_core] = 40
                            
                            # 将吞噬外壳后的核心传给下文，用于最终的 50HU 强制锁死
                            metal_core_mask = expanded_metal_core
                        else:
                            pre_s_work = pre_s
                            
                        # 使用纯净版 pre_s_work 计算用于去骨引擎的 cache 与 diff_pos
                        diff_pos_work = np.clip(post_s - pre_s_work, 0, None)
                        ...
                        # 解剖定位：用原始全序列坐标系计算 z_ratio，避免预标注裁切后
                        # total_slices 缩短导致起点 z_ratio=0 被误判为颅底。
                        # az0 为预标注裁切的全局偏移（未裁切时为 0，无副作用）。
                        ...
                        cache.z_ratio = z_ratio   # 注入z_ratio供k估计分区使用
                        bone_zones = detect_problematic_bone_zones(pre_s_work, z_ratio)

                        if opt_level == 'none':
                            res = quality_subtraction_cached(pre_s_work, post_s, adj_bone, adj_vessel, cache) if opt['quality_mode'] else fast_subtraction(pre_s_work, post_s, adj_bone, adj_vessel, body=cache.body, cache=cache)
                        elif opt_level == 'light':
                            res = optimized_subtraction_light_v3(pre_s_work, post_s, adj_bone, adj_vessel, cache)
                        elif opt_level == 'standard':
                            res = optimized_subtraction_standard_v3(pre_s_work, post_s, adj_bone, adj_vessel, cache, spacing_xy, z_ratio)
                        else:
                            res = optimized_subtraction_deep_v3(pre_s_work, post_s, adj_bone, adj_vessel, cache, spacing_xy, spacing_zyx, z_ratio)
                        ...
                        if zone == 'skull_base':
                            ...
                            # 颅顶/眶板也需要骨抑制，但只用新增的两个zone（orbital_plate/convexity_thin），
                            # 跳过岩骨/颅底/下颌（这些zone在非skull_base层数据为空，np.any()直接返回False，无额外开销）
                            gain = np.ones_like(res, dtype=np.float32)
                            gain = enhanced_bone_suppression_v299(pre_s_work, diff_pos_work, gain, bone_zones, adj_bone, adj_vessel)
                            res = res * gain
                        ...
                        # 【三维立体打击 3/3 后置：三维神来之笔】
                        # 金属核心在最终图像强制赋值 50 HU，彻底规避噪点爆光，呈现幽灵半透明轮廓供后期 VR 完美重建！
                        if metal_core_mask is not None:
                            res_scaled[metal_core_mask] = 50.0
                ...
                # ── 写出前准备：生成新的 FrameOfReferenceUID ────────────────
                # 减影结果是全新序列，必须与原平扫/增强序列的参考坐标系解耦。
                # 若继承原始 FrameOfReferenceUID，医学软件会把减影序列与原序列
                # 视为同一坐标系内的文件并尝试合并排序，发现 InstanceNumber
                # 不连续（选层时从az0开始而非1）或 SOP UID 冲突就报错。
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.log("头颅 CTA 减影-whw 版 V4.5.4 ")
        self.log("=" * 50)
        self.log("V4.5.4 【三维立体打击】终极破局版操作说明:")
        self.log("针对弹簧圈/动脉瘤夹，通过金属保护区实现三效合一：")
        self.log("1. 严苛验骨：提高门槛强力断桥，基底动脉完美保卫，不再断裂！")
        self.log("2. 寻核与吞噬：向外膨胀吃掉梯度晕影，彻底消灭刺眼外壳！")
        self.log("3. 三维神来之笔：金属核心强制锁死50HU，供后期VR完美半透明上色！")
        self.log("=" * 50)
