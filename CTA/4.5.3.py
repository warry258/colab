    ...
    def run(self):
        ...
        try:
            ...
            for chunk_idx in range(num_chunks):
                ...
                self.log.emit("\n[1/7] 读取平扫序列 (当前段)...")
                update_prog(5)
                safe_pre_files = [_get_short_path(f) for f in chunk_pre_files]
                pre_img = sitk.ReadImage(safe_pre_files, sitk.sitkInt16)
                ...
                safe_post_files = [_get_short_path(f) for f in chunk_post_files]
                post_img = sitk.ReadImage(safe_post_files, sitk.sitkInt16)
