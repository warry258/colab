...
from PyQt5.QtGui import QImage, QPixmap, QPainter, QColor, QPen, QBrush, QPolygonF, QFont, QIcon
...
class HematomaCanvas(QWidget):
    volume_changed = pyqtSignal(float, float, float, int, float, float, float)
    ...
    def __init__(self, parent=None):
        ...
        self.magic_wand_lower = 50
        self.magic_wand_upper = 85
        self.last_seed = None 
    ...
    def _calculate_volume(self):
        ...
        if voxel_count > 0:
            ...
            # ----------------------------------------------------------
            # 【粗估】模拟临床医生手工方法：
            #   C_raw = (末层IPP - 首层IPP) 的层厚 = (n-1) × 层间距
            #   医生直接用首末层 ImagePositionPatient 相减，等价于此。
            #   不做向量投影，与本程序"校正版"存在一个层厚的系统性偏差。
            # ----------------------------------------------------------
            n_hematoma = int(np.count_nonzero(z_counts))
            if n_hematoma >= 2:
                C_raw_cm = (n_hematoma - 1) * self.spacing_zyx[0] / 10.0
            else:
                C_raw_cm = n_hematoma * self.spacing_zyx[0] / 10.0
            tada_raw_ml = (A_cm * B_cm * C_raw_cm) / 2.0
        else:
            tada_raw_ml = 0.0

        self.volume_changed.emit(volume_ml, tada_ml, tada_raw_ml, z_max_idx, A_cm, B_cm, C_cm)
...
class HematomaApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("智能血肿体积计算工具 ")
        ...
        # 分隔线标签
        sep_label = QLabel("─────────── 多田公式估算 (ABC/2) ───────────")
        sep_label.setStyleSheet("color: #555; font-size: 8pt;")
        sep_label.setAlignment(Qt.AlignCenter)

        # 第二行：粗估（与医生手工一致：末层IPP − 首层IPP，不加首末半层）
        self.lbl_tada_raw = QLabel("多田估算（粗估）: 0.00 mL\n(等待提取...)")
        self.lbl_tada_raw.setStyleSheet(
            "color: #f0a500; font-size: 10.5pt; font-weight: bold;"
            "background: #2a2000; border-radius: 4px; padding: 4px 6px;")
        self.lbl_tada_raw.setContentsMargins(2, 2, 2, 2)
        self.lbl_tada_raw.setAlignment(Qt.AlignCenter)
        self.lbl_tada_raw.setToolTip(
            "与临床医生手工计算一致：\n"
            "  C = (末层IPP − 首层IPP) = (层数−1) × 层间距\n"
            "  不含首末半层，等价于医生在工作站直接量取总层厚。\n"
            "  此值便于与医生核对，但理论上偏低约一个层厚。")

        # 第三行：校正估算（正确向量投影，含首末完整层厚）
        self.lbl_tada = QLabel("多田估算（校正）: 0.00 mL\n(等待提取...)")
        self.lbl_tada.setStyleSheet("color: #7fb3d3; font-size: 10pt; font-weight: normal;")
        self.lbl_tada.setContentsMargins(0, 0, 0, 10)
        self.lbl_tada.setAlignment(Qt.AlignCenter)
        self.lbl_tada.setToolTip(
            "经向量投影校正的多田公式：\n"
            "  C = 层数 × 平均物理层间距（向量投影）\n"
            "  含首末完整层厚，理论值更精确，\n"
            "  但比医生手工值约多一个层厚，请知悉。")
        ...
        res_layout.addWidget(sep_label)
        res_layout.addWidget(self.lbl_tada_raw)
    ...
    def load_selected_series(self):
        ...
        try:
            ...
            self.lbl_tada_raw.setText("多田估算（粗估）: 0.00 mL\n(等待提取...)")
            self.lbl_tada.setText("多田估算（校正）: 0.00 mL\n(等待提取...)")
    ...
    def on_volume_changed(self, vol_ml, tada_ml, tada_raw_ml, z_idx, A, B, C):
        self.lbl_volume.setText(f"{vol_ml:.2f} mL")
        if vol_ml > 0:
            # 粗估：C = (n-1)×层间距，与医生末层IPP−首层IPP一致
            self.lbl_tada_raw.setText(
                f"多田估算（粗估）: {tada_raw_ml:.2f} mL\n"
                f"（末层−首层 IPP 量法）")
            # 校正：C = n×层间距（含首末完整层）
            self.lbl_tada.setText(
                f"多田估算（校正）: {tada_ml:.2f} mL  (层 {z_idx + 1})\n"
                f"[ A: {A:.2f} cm | B: {B:.2f} cm | C: {C:.2f} cm ]")
        else:
            self.lbl_tada_raw.setText("多田估算（粗估）: 0.00 mL\n(等待提取...)")
            self.lbl_tada.setText("多田估算（校正）: 0.00 mL\n(等待提取...)")
