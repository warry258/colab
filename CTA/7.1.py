...
class AnnotationData:
    ...
    coil_seed_mask: Optional[np.ndarray] = None  # 保留字段，暂不使用
    coil_threshold: float = 1700.0
    z_range_start: int = 0
    z_range_end: int = -1
    series_uid: str = ""
...
def get_metal_wand_threshold(wc: float, ww: float, seed_hu: Optional[float] = None) -> float:
    """
    金属魔棒阈值：
    - 不再直接等于窗宽上沿（wc+ww/2 ≈ 400 HU，那是骨窗，会连入颅骨）
    - 对瘤夹/弹簧圈设置硬下限 ≥ 1500 HU
    - 若点击到超高HU金属，阈值随seed HU自适应上调，但保证种子点本身不被排除
    """
    ui_thr = float(wc + ww / 2.0)
    base = max(1500.0, ui_thr)

    if seed_hu is None:
        return base

    # 随seed HU自适应，但保证阈值不会高到把种子点自己排除
    adaptive = max(base, min(seed_hu * 0.72, seed_hu - 120.0))
    adaptive = min(adaptive, seed_hu - 30.0)
    return max(1200.0, adaptive)


def make_ellipsoid_structure_3d(radius_mm: float, spacing_zyx: tuple) -> np.ndarray:
    """
    按物理尺寸构造3D椭球结构元，避免z/x/y各向异性导致形态学畸形。
    spacing_zyx = (z, y, x)
    """
    sz, sy, sx = [float(v) for v in spacing_zyx]
    rz = max(1, int(np.ceil(radius_mm / max(sz, 1e-6))))
    ry = max(1, int(np.ceil(radius_mm / max(sy, 1e-6))))
    rx = max(1, int(np.ceil(radius_mm / max(sx, 1e-6))))

    zz, yy, xx = np.ogrid[-rz:rz+1, -ry:ry+1, -rx:rx+1]
    se = ((zz * sz / radius_mm) ** 2 +
          (yy * sy / radius_mm) ** 2 +
          (xx * sx / radius_mm) ** 2) <= 1.0
    return se.astype(bool)


def get_mask_bbox_3d(mask: np.ndarray, pad: int = 0):
    if mask is None or not mask.any():
        return None
    zs, ys, xs = np.where(mask)
    z0 = max(0, int(zs.min()) - pad)
    z1 = min(mask.shape[0] - 1, int(zs.max()) + pad)
    y0 = max(0, int(ys.min()) - pad)
    y1 = min(mask.shape[1] - 1, int(ys.max()) + pad)
    x0 = max(0, int(xs.min()) - pad)
    x1 = min(mask.shape[2] - 1, int(xs.max()) + pad)
    return z0, z1, y0, y1, x0, x1


def match_shape_3d(arr: np.ndarray, target_shape: tuple) -> np.ndarray:
    """
    ndimage.zoom 前后尺寸可能有±1误差，做安全裁剪/补零对齐。
    """
    out = np.zeros(target_shape, dtype=arr.dtype)
    z = min(target_shape[0], arr.shape[0])
    y = min(target_shape[1], arr.shape[1])
    x = min(target_shape[2], arr.shape[2])
    out[:z, :y, :x] = arr[:z, :y, :x]
    return out


def largest_connected_component_3d(mask: np.ndarray) -> np.ndarray:
    if mask is None or not mask.any():
...
def estimate_clip_render_hu(pre_vol: np.ndarray,
                            clip_mask: np.ndarray,
                            fallback_hu: float = 2600.0,
                            log_cb=None) -> float:
    """
    估计VR用的目标金属HU。
    取mask内实测高HU值的95百分位，截断至2200~3200之间，保持VR稳定。
    """
    if clip_mask is None or not clip_mask.any():
        return float(fallback_hu)

    vals = pre_vol[clip_mask].astype(np.float32)
    ...
    target = float(np.percentile(vals, 95))
    target = float(np.clip(target, 2200.0, 3200.0))

    if log_cb:
        log_cb(f"  [瘤夹] VR目标HU估计：{target:.0f} HU")

    return target


def cast_solid_clip_3d(result_vol: np.ndarray,
                       coil_core_vol: np.ndarray,
                       spacing_zyx: tuple,
                       target_hu: float = 2600.0,
                       log_cb=None) -> np.ndarray:
    """
    适合VR的瘤夹铸造（方案一核心修复）：
    - 核心填充 target_hu（2200~3200 HU），始终在VR金属显示带内
    - 边缘过渡从 shell_floor_hu（≥1200 HU）开始，平滑后边缘体素也不会跌出金属区
    - 局部3×超采样平滑几何边界，降采样后焊回，真正消除体素化锯齿
    - 用 maximum 写回，不压低周围已有的血管高亮
    """
    if coil_core_vol is None or not coil_core_vol.any():
        return result_vol

    target_hu = float(np.clip(target_hu, 2200.0, 3200.0))
    shell_floor_hu = float(np.clip(max(1400.0, target_hu * 0.55), 1200.0, target_hu - 200.0))

    if log_cb:
        log_cb(f"\n  [瘤夹重塑] 启动 3D VR金属铸造：核心 {target_hu:.0f} HU，壳层下限 {shell_floor_hu:.0f} HU")

    # ── 1. 温和的mask抛光，避免原始布尔mask太锯齿 ──
    polish_se = make_ellipsoid_structure_3d(0.7, spacing_zyx)
    molded_mask = ndimage.binary_closing(coil_core_vol, structure=polish_se, iterations=1)
    if molded_mask.any():
        molded_mask = largest_connected_component_3d(molded_mask)
    else:
        molded_mask = coil_core_vol

    bbox = get_mask_bbox_3d(molded_mask, pad=4)
    if bbox is None:
        return result_vol

    z0, z1, y0, y1, x0, x1 = bbox
    local_mask = molded_mask[z0:z1+1, y0:y1+1, x0:x1+1]

    # ── 2. 局部超采样，仅在小包围盒里做，节省内存 ──
    up = 3
    hr_mask = ndimage.zoom(
    ...
    smooth_mm = 0.55
    sigma_hr = (
        max(0.8, smooth_mm / (spacing_zyx[0] / up)),
        max(0.8, smooth_mm / (spacing_zyx[1] / up)),
        max(0.8, smooth_mm / (spacing_zyx[2] / up)),
    )
    alpha_hr = ndimage.gaussian_filter(hr_mask, sigma=sigma_hr)
    if alpha_hr.max() > 0:
        alpha_hr /= alpha_hr.max()

    # ── 4. 降采样回原分辨率 ──
    alpha = ndimage.zoom(
        alpha_hr,
        zoom=(
            local_mask.shape[0] / alpha_hr.shape[0],
            local_mask.shape[1] / alpha_hr.shape[1],
            local_mask.shape[2] / alpha_hr.shape[2],
        ),
        order=1
    )
    alpha = match_shape_3d(alpha, local_mask.shape)
    alpha = np.clip(alpha, 0.0, 1.0)

    # 核心实体强制满值，边缘才做过渡
    alpha[local_mask] = 1.0

    # ── 5. 按alpha生成HU值，始终在金属VR显示带内 ──
    clip_patch = np.zeros_like(alpha, dtype=np.float32)
    valid = alpha > 0.03
    clip_patch[valid] = shell_floor_hu + alpha[valid] * (target_hu - shell_floor_hu)
    clip_patch[local_mask] = target_hu

    # ── 6. 用 maximum 焊回，不压低周围已有高亮血管 ──
    patch_raw = result_vol[z0:z1+1, y0:y1+1, x0:x1+1].astype(np.float32)
    patch_raw = np.maximum(patch_raw, clip_patch)
    result_vol[z0:z1+1, y0:y1+1, x0:x1+1] = np.clip(patch_raw, -32768, 32767).astype(np.int16)

    if log_cb:
        log_cb(f"  [瘤夹重塑] 完成：覆盖 {int(valid.sum())} 体素，局部超采样×{up}，"
               f"HU范围 {shell_floor_hu:.0f}~{target_hu:.0f}")
                    ...
                    if seed_hu < 1200.0:
                        if self.on_status_message:
                            self.on_status_message(
                                f"种子点HU={seed_hu:.0f} 偏低，不像金属；请点击更亮的瘤夹/弹簧圈区域"
                            )
                    else:
                        thr = get_metal_wand_threshold(self.wc, self.ww, seed_hu)
                        if self.on_status_message:
                            self.on_status_message(f"魔棒泛洪中（核心阈值≥{thr:.0f} HU）…")
                        ...
                        if seed_label > 0:
                            ...
                            self.annotations.coil_threshold = thr
                            ...
                            if self.on_status_message:
                                self.on_status_message(
                                    f"✓ 金属核心已捕获：{n_vox} 体素，跨 {n_slices} 层（阈值≥{thr:.0f} HU）"
                                )
                        else:
                            if self.on_status_message:
                                self.on_status_message("种子点未落在高HU连通金属内，请重试")
                        self.update()

            elif event.button() == Qt.RightButton:
                if self.annotations is not None:
                    self.annotations.coil_seed_mask = None
                    self.annotations.coil_threshold = 1700.0
                self.wand_preview_mask = None
                if self.on_status_message:
        ...
        try:
            ...
            if self.pre_annotation:
                ...
                ann_rois = AnnotationData(
                    ...
                    coil_seed_mask=self.pre_annotation.coil_seed_mask,
                    coil_threshold=self.pre_annotation.coil_threshold,
                    z_range_start=0,
            ...
            for chunk_idx in range(num_chunks):
                ...
                for z in range(z_start, z_end + 1):
                    ...
                    if pre_s.max() > -500:
                        ...
                        m_core, m_halo, m_artifact = None, None, None
                        if has_metal and z in annotation_data.metal_rois:
                            ...
                            if masks: 
                                ...
                                raw_metal_core = metal_mask_z & (pre_s > 2800)
                                m_core = ndimage.binary_dilation(raw_metal_core, iterations=2) & metal_mask_z
                                strict_bone = pre_s > 400
                                strict_solid_bone = ndimage.binary_dilation(ndimage.binary_opening(strict_bone, iterations=2), iterations=2) & strict_bone
                                m_artifact = metal_mask_z & ((pre_s > 120) | (pre_s < -20)) & (~strict_solid_bone) & (~m_core)
                                m_halo = metal_mask_z & (~m_core)


                        cache = precompute_slice_cache(pre_s, real_diff, opt['quality_mode'])
                        ...
                        bone_zones = detect_problematic_bone_zones_v2(pre_s, slices_from_top, eff_conv_end)

                        if opt_level == 'none':
                            res = quality_subtraction_cached(pre_s, post_s, adj_bone, adj_vessel, cache) if opt['quality_mode'] else fast_subtraction(pre_s, post_s, adj_bone, adj_vessel, body=cache.body, cache=cache)
                        elif opt_level == 'light':
                            res = optimized_subtraction_light_v3(pre_s, post_s, adj_bone, adj_vessel, cache)
                        elif opt_level == 'standard':
                            res = optimized_subtraction_standard_v3(pre_s, post_s, adj_bone, adj_vessel, cache, spacing_xy, 0.5)
                        else:
                            res = optimized_subtraction_deep_v3(pre_s, post_s, adj_bone, adj_vessel, cache, spacing_xy, spacing_zyx, 0.5)

                        gain = np.ones_like(res, dtype=np.float32)
                        gain = enhanced_bone_suppression_v299(pre_s, real_diff, gain, bone_zones, adj_bone, adj_vessel)
                        res = res * gain
                        ...
                        rescue_mask = np.zeros_like(pre_s, dtype=bool)
                        if m_halo is not None: rescue_mask |= (m_halo & (real_diff > rescue_threshold) & (pre_s < 400))
                        
                        if rescue_mask.any():
                            res_scaled[rescue_mask] = real_diff[rescue_mask] * opt['vessel_enhance']

                        kill_mask = np.zeros_like(pre_s, dtype=bool)
                        if m_artifact is not None: kill_mask |= m_artifact
                        
                        kill_mask &= ~rescue_mask 
                        if m_core is not None: kill_mask &= ~m_core
                        
                        if kill_mask.any():
                            res_scaled[kill_mask] = 0.0

                        if m_core is not None and m_core.any():
                            res_scaled[m_core] = pre_s[m_core]
                        ...
                        # 注意：w_core 旧有3D铸造流程已废弃，由瘤夹保护区取代
...
class CervicalProcessThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # ann_rois 中逐层ROI的层号偏移量
            roi_offset = az0  # 仅在需要将 ann_rois 层号映射到 local_z 时使用
