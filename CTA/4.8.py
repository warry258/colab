"""
头颅 CTA 减影-whw 版 V4.5.4 (三维形态学终极破局版)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[定制] 移除了 血管缩放 功能
[定制] 移除了 MIP预览 功能
...
class SliceCache:
    def __init__(self):
        ...
        # z_ratio：当前层在全序列中的相对位置（0=最低层，1=最高层）
        # 供 estimate_k_by_zone 分区域估计beam hardening系数使用
        self.z_ratio = 0.5
...
def get_anatomical_zone(z_index, total_slices):
    ...
    if z_ratio < 0.25: return 'skull_base'
    elif z_ratio < 0.40: return 'willis_circle'
    else: return 'convexity'
...
def detect_problematic_bone_zones(pre_slice, z_ratio):
    ...
    # ── 颅底骨/前床突：z_ratio放宽到0.38，覆盖前床突实际层面 ──
    skull_base = (pre_f > 250) & (pre_f < 1000) if z_ratio < 0.38 else np.zeros_like(pre_slice, dtype=bool)

    # ── 下颌骨 ──
    mandible = pre_f > 200 if z_ratio < 0.18 else np.zeros_like(pre_slice, dtype=bool)

    # ── 锐利骨边缘（高梯度骨质，全层有效）──
    sharp_edge = (pre_f > 150) & (gradient > 120)

    # ── 眶板：薄骨 + 紧邻眼眶脂肪/气腔，z_ratio约0.12~0.42 ──────────
    # 眶内脂肪HU约-50~-100，气体更低。原先<-80漏掉HU在-50~-80的脂肪区域，
    # 改为<-50扩大气腔识别范围，配合dilation 4px确保骨板被覆盖。
    orbital_plate = np.zeros_like(pre_slice, dtype=bool)
    if 0.12 < z_ratio < 0.42:
        orbit_air = ndimage.binary_dilation(pre_f < -50, iterations=4)   # 眶内脂肪/气体（放宽）
        orbital_bone = (pre_f > 150) & (pre_f < 700)                     # 眶板HU范围
        orbital_plate = orbital_bone & orbit_air                          # 骨+邻气腔
        if orbital_plate.sum() > 20:
            orbital_plate = ndimage.binary_opening(orbital_plate, iterations=1)

    # ── 颅顶/穹隆薄骨：erosion 3px，覆盖外板残留 ──────────────────────
    # erosion 2px时外板（厚约2~3px）刚好在边界外，呈现同心圆残留。
    # 改为erosion 3px，把外板也纳入识别范围（厚度<6px的骨板全覆盖）。
    convexity_thin = np.zeros_like(pre_slice, dtype=bool)
    if z_ratio >= 0.38:
        bone = pre_f > 150
        bone_eroded = ndimage.binary_erosion(bone, iterations=3)
        convexity_thin = bone & ~bone_eroded    # 厚度<6px的薄骨均在此
...
def get_zone_adjusted_params(zone, bone_strength, vessel_sensitivity):
    if zone == 'skull_base': return bone_strength * 1.3, vessel_sensitivity * 1.3
    elif zone == 'willis_circle': return bone_strength * 0.75, vessel_sensitivity * 1.6
...
def estimate_k_by_zone(pre_f, post_f, bone_mask, z_ratio):
    """
    分区域估计beam hardening系数k：post_bone ≈ k * pre_bone
    物理含义：k-1 是骨HU的相对漂移率，与骨密度成正比。
    分三个解剖区域独立回归，避免颅底高HU骨污染凸面薄骨的k估计。
    ...
    # 三个区域：颅底(z<0.38)、凸面(z>=0.38)、全局兜底
    zones =[]
    if z_ratio < 0.38:
        zones.append(('skull_base', solid_bone & (pre_f > 400)))   # 颅底致密骨
        zones.append(('skull_base_med', solid_bone & (pre_f <= 400)))  # 颅底中密度骨
    else:
        zones.append(('convexity', solid_bone))  # 凸面统一处理
...
def optimized_subtraction_light_v3(pre, post_aligned, bone_strength, vessel_sensitivity, cache):
    ...
    z_ratio = getattr(cache, 'z_ratio', 0.5)   # 由调用方注入，默认凸面
    k_map, k_global = estimate_k_by_zone(pre_f, post_f, bone_mask, z_ratio)
...
class ProcessThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.log.emit("=" * 55)
            self.log.emit(f"CTA 减影 V4.5.4 (三维形态学终极破局版) | 优化:{opt_name}")
            ...
            for chunk_idx in range(num_chunks):
                ...
                if ann_rois:
                    seg_ann = AnnotationData(
                        z_range_start=0, z_range_end=chunk_depth - 1, scale_factor=ann_rois.scale_factor
                    )
                ...
                for z in range(z_start, z_end + 1):
                    ...
                    if pre_s.max() > -500:
                        ...
                        zone = get_anatomical_zone(original_global_z, original_total_slices)
                        adj_bone, adj_vessel = get_zone_adjusted_params(zone, opt['bone_strength'], opt['vessel_sensitivity'])
                        z_ratio = original_global_z / original_total_slices
                        cache.z_ratio = z_ratio   
                        bone_zones = detect_problematic_bone_zones(pre_s_work, z_ratio)
                ...
                for z in range(z_start, z_end + 1):
                    ...
                    ds.SeriesDescription = f"CTA Sub V4.5.4[{opt_name}]{ann_tag}"
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.init_ui()
    def init_ui(self):
        self.setWindowTitle("头颅 CTA 减影-whw 版 V4.5.4")
        ...
        self.log("头颅 CTA 减影-whw 版 V4.5.4 操作步骤")
        self.log("=" * 50)             
        self.log("1. 扫描目录自动配对")
    ...
    def on_pre_ann_ready(self, post_vol: np.ndarray, post_files: list):
        ...
        if result == QDialog.Accepted and dlg.result_accepted:
            ...
            self.pre_ann_status.setText(f"✓ 已标注：第{z0}~{z1}层（{n}层，{roi_tip}）")
            self.pre_ann_status.setStyleSheet("color:#27ae60;font-size:9pt;font-weight:bold;")
            self.log(f"✅ 预标注完成：选定第 {z0}~{z1} 层，共 {n} 层，{roi_tip}")
            self.start_btn.setEnabled(True)
        else:
            self.pre_annotation_result = None
